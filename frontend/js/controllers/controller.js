...
myApp.controller('HomeCtrl', function ($scope,$rootScope, TemplateService, NavigationService,CsrfTokenService,Menuservice, $timeout,$http,apiService,$state,$cookies) {
    ...
    .controller('AgentdashboardCtrl', function ($scope, $rootScope,$resource,TemplateService, NavigationService,CsrfTokenService, $timeout,$http,apiService,$state,$uibModal,Menuservice,tts,$cookies,$sce,$location) {
        ...
        // Create the HTML for the room
        var roomHTML = '<button type="button" class="btn btn-primary" data-toggle="collapse" data-target="#collapseExample'+roomName+'" aria-expanded="false" aria-controls="collapseExample'+roomName+'">Private conversation with <span id="private-username-'+penPal.id+'">'+penPalName+'</span></button>' +
                        '<div class="collapse" id="collapseExample'+roomName+'"><div id="private-messages-'+penPal.id+'" style="width: 100%; height: 150px; overflow: auto; border: solid 1px #666; padding: 5px; margin: 10px 0;border-radius:5px;"></div>'+
                        '<input id="private-message-'+penPal.id+'" class="form-control pvtmsg"/> <button class="btn btn-primary" id="private-button-'+penPal.id+'">Send message</button"></div>';
    ...
    .controller('ChatCtrl', function ($scope, $rootScope,TemplateService, NavigationService,CsrfTokenService, $timeout,$http,apiService,$state,$uibModal,Menuservice,tts,$cookies,$sce) {
        ...
        $rootScope.sendMsgtoagent = function(msg) {
            ...
            io.socket.get('/user', function (users){
                ...
                if(newuser.length > 0)
                {
                    if(!$rootScope.lastagent)
                        $rootScope.lastagent = newuser[0].sid;
                    else {

                    }
                    //console.log(newuser,"users");
                    addMessageToConversation(window.me.id, newuser[0].id, msg);
                    io.socket.post('/chat/private', {to: newuser[0].id, msg: msg});
                }
            });
